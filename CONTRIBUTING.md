...
of the code. We are using uv to keep our development environment up to date. Please follow the instructions here
https://docs.astral.sh/uv/ to install uv. Next, pull the repo to your local machine, open a terminal window
and navigate to the top directory of this package. Run the command
`uv sync --group contributor --group dev` to install all required tools and dependencies for contributing to this package.
...
- isort: Sorts the inputs, can be used from the command line `uv run isort .`, use the `--check` flag if you do not want to
reformat the import statements in place but just want to check if imports need to be reformatted.
- black: Formats the code based on PEP standards, can be used from the command line: `uv run black .`, use the `--check` flag
...
but will not be able to replace any text, `uv run pydocstyle .`
...
through the command line `uv run pytest .`, use the `--cov-report term-missing` flag to show which lines are missing in the
