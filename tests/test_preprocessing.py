...
def fix_time_bin_edges(sensor_group):
    ...
    time_bin_edges = pd.arrays.DatetimeArray(pd.date_range(min_time, max_time, freq="120s"))
...
def fix_block_times(sensor_group):
    ...
    block_times = pd.arrays.DatetimeArray(pd.date_range(min_time, max_time, freq="1200s"))
    return block_times
...
def fix_meteorology(request, sensor_group):
    ...
    meteorology.time = pd.arrays.DatetimeArray(pd.date_range(min_time, max_time, freq="1s"))
